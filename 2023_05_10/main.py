...
while(True):
    ...
    if (Message.Ctr.WorkMode==1):#if(end - start < 5000):# 识别顺光黄色条形码
        ...
        Yellow_Detect_Exposure.yellow_Detect(img,(48, 99, -36, 10, 28, 80))
         #LED(1).on()
         #LED(2).off()
         #LED(3).off()
        ...
        QRandBAR.BarcodePhoto()
         ...
         #LED(2).on()
         #LED(3).off()
    elif (Message.Ctr.WorkMode==3):#elif(end - start < 20000): # 寻找红色杆子
        #print("寻找红色杆子模式！")
        ...
        Red_Detect.red_Detect(img)#该函数单独测试也要加曝光设置，但还没加？？？
         #LED(1).off()
         #LED(2).off()
         #LED(3).on()
    elif(Message.Ctr.WorkMode==5):#elif(end - start < 25000): # 二维码拍照
        #print("二维码拍照模式！")
        ...
        QRandBAR.QRcodePhoto()
         ...
         #LED(2).on()
         #LED(3).on()
    elif(Message.Ctr.WorkMode==6):#elif(end - start < 35000):  # 红色绕杆
        #print("红色绕杆模式！")
    ...
    elif(Message.Ctr.WorkMode==7):#elif(end - start < 40000):  # 背光识别黄色
        #print("背光识别黄色模式！")
        if(work_mode_7_flag == True):
            ...
            sensor.set_auto_exposure(False,
                exposure_us=int(current_exposure_time_in_microseconds * EXPOSURE_TIME_SCALE_YELLOW_1))
            Yellow_Detect_Exposure.alert_open() # 再次打开黄灯闪烁控制定时器
            # 把上一个模式的灯关了，防止影响黄色识别的标志灯光
            LED(1).off()
            LED(2).off()
            LED(3).off()

        Yellow_Detect_Exposure.yellow_Detect(img,(25, 80, -28, 12, 24, 67))
    elif (Message.Ctr.WorkMode==8):
        if(work_mode_8_flag == True):
            work_mode_8_flag = False
            Yellow_Detect_Exposure.alert_close()    # 关闭控制黄灯闪烁的定时器
            # 设置曝光值为红色适应曝光值
            sensor.set_auto_exposure(False,
                exposure_us=int(current_exposure_time_in_microseconds * EXPOSURE_TIME_SCALE_RED))
            # 再关一次灯确保上一个模式的灯不会对此模式的标志灯光造成影响
