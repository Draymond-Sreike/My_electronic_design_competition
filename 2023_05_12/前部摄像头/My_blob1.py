...
def DotCheck():
    ...
    if max_blob:
        ...
        if (CENTER_ROI[0] < max_blob.cx() and  max_blob.cx() < (CENTER_ROI[0]+CENTER_ROI[2])
            and (CENTER_ROI[1] < max_blob.cy() and  max_blob.cy() < CENTER_ROI[1]+CENTER_ROI[3]):
            # 进入中心区域亮红灯
            LED(1).on()
            LED(2).off()
            LED(3).off()
    ...
    #数据打包不同，需要另写
    Message.UartSendData(Message.DotDataPack(Dot.color,Dot.flag,Dot.x,Dot.y,0x00))
    return Dot.flag
