...
def yellow_Detect(img):
    ...
    if blobs:
        ...
        if (CENTER_ROI[0] < max_blob[5] and max_blob[5] < (CENTER_ROI[0] + CENTER_ROI[2])):
            print("黄色物体进入中心区域！","距离(平均)", distance_average, "cm")
            # 距离20cm的时候测量值是最准的，距离40cm的时候测量值约为34cm
            print("长：", max_blob[2], "宽：", max_blob[3], "距离(判L)：", distance_L, "距离(判W)", distance_W,"距离(平均)", distance, "cm")
            Message.UartSendData(Message.My_Pack(0x01, distance_average,0))
            # 黄灯表示物体进入中心区域
            LED(1).on()
            LED(2).on()
            LED(3).off()
